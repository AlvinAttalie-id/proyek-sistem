            ...
            <div class="col-12">
                <div class="card mb-4">
                    ...
                    <div class="form-row w-full px-4 flex flex-wrap">
                        <form action="{{ route('anggaran-masuk.index') }}" method="get" class="d-inline">
        ...
        <div class="row">
            <div class="col-12">
                @if(session('success'))
                    ...
                    @endif
                <div class="card mb-4">
                    <table class="table align-items-center mb-0 table-hover" id="anggaranTable">
                        <thead>
                            <tr>
                                ...
                                <th class="text-uppercase text-dark text-center text-xs font-weight-bolder">Tanggal Masuk</th>
                        ...
                        <tbody>
                            @foreach($anggaranMasuk as $id => $data)
                            <tr>
                                <td class="text-center text-xs">{{ $id + 1 }}</td>
                                <td class="text-center text-xs">{{ $data->kode_proyek }}</td>
                                <td class="text-center text-xs">{{ $data->tanggal_masuk->format('d-m-Y') }}</td>
                                <td class="text-center text-xs">Rp. {{ number_format($data->jumlah_masuk, 2, ',', '.') }}</td>
                                ...
                                <td class="text-center text-xs">
                                    ...
                                    <form action="{{ route('anggaran-masuk.destroy', ['anggaran_masuk' => $data->id]) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="{{ route('anggaran-masuk.update', ['anggaran_masuk' => $data->id]) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <div class="row mt-4">
                                                    <div class="col-md-6">
                                                    ...
                                                    <div class="col-md-6">
                                                        <label for="edit_tanggal_masuk{{ $data->id }}" class="form-control-label">Tanggal Masuk</label>
                                                        <div class="form-group">
                                                            <input type="date" name="tanggal_masuk" class="form-control" id="edit_tanggal_masuk{{ $data->id }}" value="{{ $data->tanggal_masuk->format('Y-m-d') }}" required>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                                                        <label for="edit_jumlah_masuk{{ $data->id }}" class="form-control-label">Jumlah Masuk</label>
                                                        <div class="form-group">
                                                            <input type="number" step="0.01" name="jumlah_masuk" class="form-control" id="edit_jumlah_masuk{{ $data->id }}" value="{{ $data->jumlah_masuk }}" required>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6">
                    ...
                    <div class="d-flex justify-content-between">
                        <!-- Pagination Links -->
                        <div class="mt-3">
                            {{ $anggaranMasuk->links('pagination::bootstrap-5') }}
                        </div>
                        <!-- Button trigger modal -->
                ...
                <form action="{{ route('anggaran-masuk.store') }}" method="post">
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <input type="number" step="0.01" name="jumlah_masuk" class="form-control" id="jumlah_masuk" required>
                            </div>
                        </div>
                        <div class="col-md-6">
